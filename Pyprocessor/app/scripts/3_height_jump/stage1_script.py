...
# Constants
MODEL_PATH = os.path.join("height_jump", "stages", "stage1", "models", "highjump_stage1.keras")
...
NUM_FEATURES = 2  # Left foot and right foot binary values
...
def extract_keypoints_with_upright_posture(video_path):
    """
    Extracts keypoints based on upright posture for the given video.
    """
    keypoints = []
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks:
            ...
            # Extract keypoints for left and right shoulder, hip, and ankle
            ...
            keypoints.append([int(left_upright), int(right_upright)])
...
def process_video(video_path):
    ...
    try:
        ...
        keypoints = extract_keypoints_with_upright_posture(video_path)
        if not keypoints:
...
if __name__ == "__main__":
    test_video_path = os.path.join("height_jump", "stages", "stage1", "test_videos", "0_user3.mp4")
