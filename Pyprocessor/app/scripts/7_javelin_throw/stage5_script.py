...
# Constants
MODEL_PATH = os.path.join("spear_throwing", "stages", "stage5", "models", "javelin_stage5.keras")
...
# Function to determine if the throw is initiated through hip-torso involvement
def is_throw_initiated(hip, torso, shoulder):
    """
    Determines if the throw is initiated by analyzing the relative positions
    of the hip, torso, and shoulder.
    The hip should rotate forward before the torso and shoulder follow.
    """
    hip_x, _ = hip
    torso_x, _ = torso
    shoulder_x, _ = shoulder

    # Throw initiated if hip moves ahead of torso and torso moves ahead of shoulder
    return hip_x > torso_x > shoulder_x

# Function to extract keypoints for throw initiation analysis
def extract_keypoints(video_path):
    """
    Extract keypoints to analyze the throw initiation.
    """
    keypoints = []
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks:
            ...
            # Extract relevant keypoints for throw analysis (hip, torso, and shoulder)
            right_hip = [landmarks[mp_pose.PoseLandmark.RIGHT_HIP].x,
                         landmarks[mp_pose.PoseLandmark.RIGHT_HIP].y]
            torso = [landmarks[mp_pose.PoseLandmark.NOSE].x,
                     landmarks[mp_pose.PoseLandmark.NOSE].y]
            right_shoulder = [landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER].x,
                              landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER].y]

            # Check if the throw is initiated through hip-torso involvement
            throw_initiated = is_throw_initiated(right_hip, torso, right_shoulder)

            # Store the binary value (0 or 1) for analysis
            keypoints.append([int(throw_initiated)])
...
if __name__ == "__main__":
    test_video_path = os.path.join("spear_throwing", "stages", "stage5", "test_videos", "1_user5.mp4")
